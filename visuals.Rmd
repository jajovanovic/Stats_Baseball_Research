...
Instead of writing the full `geom_rect()` code in every single graph we can make a function. The function can be added to a graph like any other layer (for example, `geom_point()`) by writing `geom_zone()`. Once you read this function into an R session, you can call this function. 
...
`geom_zone()` will add an average zone unless otherwise specified. To customize the zone to a specific player, calculate their zone and plug it into the function using the arguments `top = ` and `bottom = `.
...
Here is another strike zone plot using our new function `geom_zone()`. We will use the output from above to make Judge's strike zone.
This graph is faceted by infield shift and colored by outfield shift.
...
#### George Kirby's Debut by Pitch Type {.tabset}

**These are different options for Kirby's pitches. I figured we'll only keep one in but couldn't decide between which so I wanted your opinion.** 
...
ADD EXPLANATION 
I'm trying to figure out if there's a way to have the pitchers names show up without me having to look up each individual pitcher and recode it
...
game %>% 
  ...
  theme_bw() +
  facet_wrap(~pitcher, nrow = 2)
```


## Spray Chart

Spray charts show the location of balls hit into play. One game usually produces around 250-300 rows of data. To get more data points, we will use all games played on July 2, 2022 and July 3, 2022.
...
spray %>% 
  ggplot(aes(x = hc_x, y = -hc_y, color = pitch_name)) +
  geom_point(alpha = .5) +
  labs(title = "Spray Chart Without Field Lines", subtitle = "All MLB Games July 2 & July 3, 2022", color = "Pitch") +
  theme_void() 
```
...
From the concentration of points you can get an idea of the shape of the field. 
It would be nice to see the lines of the baseball field on the spray chart. We can create a function to do this!
...
`geom_spray()` plots the lines for an _average_ ballpark. Each park can differ a bit in their dimensions, but overall have this shape.
...
spray %>% 
  ...
  labs(title = "Spray Chart With Field Lines", subtitle = "All MLB Games July 2 & July 3, 2022", color = "Pitch") +
  theme_void()
```
...
judge %>% 
  ...
  geom_spray(aes(x = hc_x, y = -hc_y)) +
  geom_point(alpha = .5, aes(color = events)) +
  labs(title = "Spray Chart by Outcome", subtitle = "Aaron Judge 2022 Season") +
  theme_void() +
...
should I talk about the different color palettes (viridis, RColorBrewer)?
