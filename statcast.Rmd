...
**attempt to get multiple seasons at once**
```{r}
...
**NEW:** 
I tried to use `geom_label()` to show the player with the max and min but it isn't turning out right - it changes the label for hit speed to letters instead of the dots, name is getting cut off (can it be wrapped?)
...
sc_lead_evb %>% 
  ...
  ggplot(aes(x = max_distance, y = max_hit_speed, color = avg_distance, size = avg_hit_speed)) +
  geom_point(alpha = .5) +
  geom_text(data = sc_lead_evb %>% 
               filter(max_distance %in% range(max_distance)),
             aes(label = paste(first_name, last_name)), nudge_y = 0.5) +
  labs(title = "Hitting Leaders", x = "Max Distance", y = "Max Hit Speed",
       color = "Average Distance", size = "Average Hit Speed") +
  scale_color_viridis_b() +
  theme_bw()
